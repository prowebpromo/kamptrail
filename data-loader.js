...
(function() {
  ...
  function applyFilters(sites, filters) {
    ...
    return sites.filter(site => {
      ...
      if (filters.searchText && filters.searchText.trim()) {
        ...
        if (!name.includes(searchLower) && !state.includes(searchLower)) return false;
      }
  ...
  window.KampTrailData = {
    ...
    setUserLocation(lat, lng) {
      ...
      // Re-render markers to update popups with distance
    ...
    async shareCampsite(id) {
      ...
      const p = site.properties;
      const [lng, lat] = site.geometry.coordinates;
      const name = p.name || 'Unnamed Campsite';
      const costText = p.cost === 0 || p.cost === null ? 'FREE' : `$${p.cost}/night`;
      ...
      const shareData = {
        title: `🏕️ ${name}`,
        text: `Check out this campsite on KampTrail!\n\n${name}${distanceText}\nCost: ${costText}\nType: ${p.type || 'Unknown'}\n\nView on map:`,
        url: googleMapsUrl
      };
    ...
    fallbackShare(shareData) {
      ...
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(text).then(() => {
          if (window.showToast) {
            showToast('📋 Campsite info copied to clipboard!', 'success');
          }
        }).catch(err => {
